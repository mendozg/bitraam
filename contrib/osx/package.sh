...
if ! which ${genisoimage} > /dev/null 2>&1; then
    mkdir -p /tmp/bitraam-macos
    cd /tmp/bitraam-macos
...
rm -rf /tmp/bitraam-macos/image > /dev/null 2>&1
mkdir /tmp/bitraam-macos/image/
cp -r $1 /tmp/bitraam-macos/image/
...
${genisoimage} \
    ...
    -dir-mode 0755 \
    -apple \
    -o Bitraam_uncompressed.dmg \
    /tmp/bitraam-macos/image || fail "Unable to create uncompressed dmg"

dmg dmg Bitraam_uncompressed.dmg bitraam-$VERSION.dmg || fail "Unable to create compressed dmg"
rm Bitraam_uncompressed.dmg

echo "Done."
sha256sum bitraam-$VERSION.dmg
