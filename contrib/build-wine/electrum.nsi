...
;Variables
  ...
  !define PRODUCT_NAME "Bitraam"
  !define PRODUCT_WEB_SITE "https://github.com/mendozg/bitraam"
  !define PRODUCT_PUBLISHER "Bitraam Technologies GmbH"
...
;General
  ...
  ;Name and file
  Name "${PRODUCT_NAME}"
  OutFile "dist/bitraam-setup.exe"
...
;Interface Settings
  ...
  !define MUI_ICON "..\..\bitraam\gui\icons\bitraam.ico"
...
Section
  ...
  File /r "dist\bitraam\*.*"
  File "..\..\bitraam\gui\icons\bitraam.ico"
  ...
  CreateShortCut "$DESKTOP\${PRODUCT_NAME}.lnk" "$INSTDIR\bitraam-${PRODUCT_VERSION}.exe" ""

  ;Create start-menu items
  DetailPrint "Creating start-menu items..."
  CreateDirectory "$SMPROGRAMS\${PRODUCT_NAME}"
  CreateShortCut "$SMPROGRAMS\${PRODUCT_NAME}\Uninstall.lnk" "$INSTDIR\Uninstall.exe" "" "$INSTDIR\Uninstall.exe" 0
  CreateShortCut "$SMPROGRAMS\${PRODUCT_NAME}\${PRODUCT_NAME}.lnk" "$INSTDIR\bitraam-${PRODUCT_VERSION}.exe" "" "$INSTDIR\bitraam-${PRODUCT_VERSION}.exe" 0
  ...
  WriteRegStr HKCU "Software\Classes\lightning\shell\open\command" "" "$\"$INSTDIR\bitraam-${PRODUCT_VERSION}.exe$\" $\"%1$\""
  ...
  WriteRegStr HKCU "${PRODUCT_UNINST_KEY}" "DisplayIcon" "$INSTDIR\bitraam.ico"
