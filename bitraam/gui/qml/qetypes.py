...
class QEAmount(QObject):
    """Container for bitcoin amounts that can be passed around more
       ...
       on (milli)sitashi amounts! 'int' in QML-javascript seems to be larger than 32 bit, and
    ...
    def __init__(self, *, amount_sat: int = 0, amount_msit: int = 0, is_max: bool = False, from_invoice=None, parent=None):
        super().__init__(parent)
        self._amount_sat = int(amount_sat) if amount_sat is not None else None
        self._amount_msit = int(amount_msit) if amount_msit is not None else None
        self._is_max = is_max
        if from_invoice:
            inv_amt = from_invoice.get_amount_msit()
            if inv_amt == '!':
                self._is_max = True
            elif inv_amt is not None:
                self._amount_msit = int(inv_amt)
    ...
    @pyqtProperty('qint64', notify=valueChanged)
    def msitsInt(self):
        if self._amount_msit is None:  # should normally be defined when accessing this property
            self._logger.warning('amount_msit is undefined, returning 0')
            return 0
        return self._amount_msit

    @msitsInt.setter
    def msitsInt(self, msits):
        if self._amount_msit != msits:
            self._amount_msit = msits
            self.valueChanged.emit()
    ...
    @pyqtProperty(bool, notify=valueChanged)
    def isEmpty(self):
        return not(self._is_max or self._amount_sat or self._amount_msit)
    ...
    def copyFrom(self, amount):
        ...
        self.msitsInt = amount.msitsInt
        self.isMax = amount.isMax

    def __eq__(self, other):
        if isinstance(other, QEAmount):
            return self._amount_sat == other._amount_sat and self._amount_msit == other._amount_msit and self._is_max == other._is_max
    ...
    def __str__(self):
        ...
        if self._is_max:
            return '%s(MAX)' % s
        return '%s(sats=%d, msits=%d)' % (s, self._amount_sat, self._amount_msit)

    def __repr__(self):
        return f"<QEAmount max={self._is_max} sats={self._amount_sat} msits={self._amount_msit} empty={self.isEmpty}>"
