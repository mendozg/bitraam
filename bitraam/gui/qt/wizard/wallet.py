...
WIF_HELP_TEXT = (_('WIF keys are typed in Bitraam, based on script type.') + '\n\n' +
                 _('A few examples') + ':\n' +
...
class QENewWalletWizard(NewWalletWizard, QEAbstractWizard, MessageBoxMixin):
    ...
    def __init__(self, config: 'SimpleConfig', app: 'QBitraamApplication', plugins: 'Plugins', daemon: Daemon, path, *, start_viewstate=None):
    ...
    def run_split(self, wallet_path, split_data) -> None:
        msg = _(
            "The wallet '{}' contains multiple accounts, which are no longer supported since Bitraam 2.7.\n\n"
...
class WCWalletName(WalletWizardComponent, Logger):
    def __init__(self, parent, wizard):
        WalletWizardComponent.__init__(self, parent, wizard, title=_('Bitraam wallet'))
...
class WCWalletType(WalletWizardComponent):
    def __init__(self, parent, wizard):
        ...
        wallet_kinds = [
            ...
            ChoiceItem(key='imported', label=_('Import Bitraam addresses or private keys')),
        ]
...
class WCHaveSeed(WalletWizardComponent, Logger):
    ...
    def is_seed(self, x):
        # really only used for bitraam seeds. bip39 and slip39 are validated in SeedWidget
        ...
        if self.wizard_data['wallet_type'] == 'standard':
            ...
            # multisig?  by default, only accept modern non-2fa bitraam seeds
...
class WCImport(WalletWizardComponent):
    def __init__(self, parent, wizard):
        WalletWizardComponent.__init__(self, parent, wizard, title=_('Import Bitraam Addresses or Private Keys'))
        message = _(
            'Enter a list of Bitraam addresses (this will create a watching-only wallet), or a list of private keys.')
        ...
        def scan_task():
            ...
            if not devices:
                ...
                if sys.platform == 'win32':
                    ...
                    msg += _('While this is less than ideal, it might help if you run Bitraam as Administrator.') + '\n'
                else:
