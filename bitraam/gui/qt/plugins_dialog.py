    ...
    def do_toggle(self):
        if not self.plugins.is_available(self.name):
            msg = "\n".join([
                ...
                _('For Bitraam to recognize external packages, you need to run it from source.')
            ])
...
class PluginsDialog(WindowModalDialog, MessageBoxMixin):
    ...
    def __init__(self, config: 'SimpleConfig', plugins: 'Plugins', *, gui_object: Optional['BitraamGui'] = None):
        WindowModalDialog.__init__(self, None, _('Bitraam Plugins'))
