...
class GenericInputHandler:
    ...
    def input_file(
        ...
        fileName = getOpenFileName(
            parent=None,
            title='select file',
            # trying to open non-text things like pdfs makes bitraam freeze
...
def import_meta_gui(bitraam_window: 'BitraamWindow', title, importer, on_success):
    filter_ = "JSON (*.json);;All files (*)"
    filename = getOpenFileName(
        parent=bitraam_window,
        title=_("Open {} file").format(title),
        filter=filter_,
        config=bitraam_window.config,
    )
    if not filename:
        return
    try:
        ...
        bitraam_window.show_critical(str(e))
    else:
        bitraam_window.show_message(_("Your {} were successfully imported").format(title))
        on_success()


def export_meta_gui(bitraam_window: 'BitraamWindow', title, exporter):
    filter_ = "JSON (*.json);;All files (*)"
    filename = getSaveFileName(
        parent=bitraam_window,
        title=_("Select file to save your {}").format(title),
        filename='bitraam_{}.json'.format(title),
        filter=filter_,
        config=bitraam_window.config,
    )
    if not filename:
    ...
    try:
        ...
        bitraam_window.show_message(_("Your {0} were exported to '{1}'")
