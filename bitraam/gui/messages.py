...
It may be imported in another Bitraam wallet with the same seed."""
)

MSG_LIGHTNING_WARNING = _(
"""Bitraam uses static channel backups. If you lose your wallet file, you will need to request your channel to be force-closed by the remote peer in order to recover your funds. This assumes that the remote peer is reachable, and has not lost its own data."""
)
...
MSG_THIRD_PARTY_PLUGIN_WARNING = ' '.join([
    ...
    '<br/><br/>',
    _('Third-party plugins are not endorsed by Bitraam.'),
    _('Bitraam will not be responsible in case of theft, loss of funds or privacy that might result from third-party plugins.'),
    '<br/><br/>',
...
MSG_LN_EXPLAIN_SCB_BACKUPS = "".join([
    ...
    _("In the Bitraam mobile app, use the 'Send' button to scan this QR code."), " ",
    "\n\n",
...
MSG_CONNECTMODE_SERVER_HELP = _(
    "Bitraam connects to a unique server in order to receive your transaction history. "
    "This server will learn your wallet addresses."
)
MSG_CONNECTMODE_NODES_HELP = _(
    "In addition to your history server, Bitraam will try to maintain connections with ~10 extra servers, in order to download block headers and find out the longest blockchain. "
...
MSG_CONNECTMODE_AUTOCONNECT_HELP = _(
    "Bitraam will always use a history server that is on the longest blockchain. "
    "If your current server is unresponsive or lagging, Bitraam will switch to another server."
)

MSG_CONNECTMODE_MANUAL_HELP = _(
    "Bitraam will stay with the server you selected. It will warn you if your server is lagging."
)

MSG_CONNECTMODE_ONESERVER_HELP = _(
    "Bitraam will stay with the server you selected, and it will not connect to additional nodes. "
