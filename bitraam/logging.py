...
# try to make console log lines short... no timestamp, short levelname, no "bitraam."
...
def _shorten_name_of_logrecord(record: logging.LogRecord) -> logging.LogRecord:
    ...
    if record.name.startswith("bitraam."):
    ...
    record.name = record.name.replace("gui.qt.main_window.BitraamWindow", "gui.qt.main_window", 1)
    return record
...
def _delete_old_logs(path, *, num_files_keep: int, max_total_size: int):
    ...
    def sortkey_oldest_first(p: pathlib.PurePath):
        ...
        # - each time bitraam is launched, there will be a new basename, ordered by date
    ...
    files = sorted(
        list(pathlib.Path(path).glob("bitraam_log_*.log*")),
...
def _configure_file_logging(
    ...
    *,
    ...
    PID = os.getpid()
    _logfile_path = log_directory / f"bitraam_log_{timestamp}_{PID}.log"
...
def _process_verbosity_log_levels(verbosity):
    ...
    for filt in filters:
        ...
        if len(items) == 1:
            level = items[0]
            bitraam_logger.setLevel(level.upper())
        elif len(items) == 2:
...
#       as possible.
# Note: some users might use Bitraam as a python library and not use run_bitraam,
...
# creates a logger specifically for bitraam library
bitraam_logger = logging.getLogger("bitraam")
bitraam_logger.setLevel(logging.DEBUG)
...
def get_logger(name: str) -> _CustomLogger:
    ...
    return bitraam_logger.getChild(name)
...
def configure_logging(config: 'SimpleConfig', *, log_to_file: Optional[bool] = None) -> None:
    ...
    _logger.info(f"Bitraam version: {ELECTRUM_VERSION} - https://bitraam.org - {GIT_REPO_URL}")
