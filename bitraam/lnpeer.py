...
class Peer(Logger, EventListener):
    ...
    def maybe_update_fee(self, chan: Channel):
        ...
        def does_chan_fee_need_update(chan_feerate: Union[float, int]) -> Optional[bool]:
            if chan.has_anchors():
                # TODO: once package relay and bitraam servers with submitpackage are more common,
        ...
        if not chan.constraints.is_initiator:
            if constants.net is not constants.BitcoinRegtest:
                ...
                if ratio < 0.5:
                    # Note that we trust the Bitraam server about fee rates
            ...
            if forwarding_info:
                ...
                if payment_key not in self.lnworker.active_forwardings:
                    async def wrapped_callback():
                        ...
                        #        - malicious bitraam server could fake TxBroadcastError
