...
from bitraam.bip32 import BIP32Node, convert_bip32_strpath_to_intpath
from bitraam.logging import Logger
from bitraam.plugin import runs_in_hwd_thread
from bitraam.hw_wallet.plugin import HardwareClientBase, HardwareHandlerBase
...
class KeepKeyClientBase(HardwareClientBase, GuiMixin, Logger):
    ...
    def has_usable_connection_with_device(self):
        try:
            res = self.ping("bitraam pinging device")
            assert res == "bitraam pinging device"
    ...
    def i4b(self, x):
        if x < 0:
            # hack. workaround for https://github.com/mendozg/bitraam/issues/7779
            x += 2 ** 32
        return pack('>I', x)
