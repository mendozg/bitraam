...
class SafeTPlugin(HW_PluginBase):
    ...
    def get_safet_input_script_type(self, bitraam_txin_type: str):
    ...
    def get_safet_output_script_type(self, bitraam_txin_type: str):
        if bitraam_txin_type in ('p2wpkh', 'p2wsh'):
            ...
            if txin.is_coinbase_input():
                ...
                if for_sig:
                    ...
                    if multi := desc.get_simple_multisig():
                        ...
                        multisig = None
                    script_type = self.get_safet_input_script_type(desc.to_legacy_bitraam_script_type())
                    txinputtype = self.types.TxInputType(
    ...
    def tx_outputs(self, tx: PartialTransaction, *, keystore: 'SafeTKeyStore'):
        ...
        def create_output_by_derivation():
            ...
            assert desc
            script_type = self.get_safet_output_script_type(desc.to_legacy_bitraam_script_type())
            if multi := desc.get_simple_multisig():
                multisig = self._make_multisig(multi)
            else:
                multisig = None
