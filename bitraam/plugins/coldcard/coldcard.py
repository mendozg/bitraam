#
# Coldcard Bitraam plugin main code.
#
#
import os
import time
...
class CKCCClient(HardwareClientBase):
    def __init__(self, plugin, handler, dev_path, *, is_simulator=False):
        ...
        if is_simulator:
            self.dev = BitraamColdcardDevice(dev_path, encrypt=True)
        else:
            # open the real HID device
            hd = hid.device(path=dev_path)
            hd.open_path(dev_path)

            self.dev = BitraamColdcardDevice(dev=hd, encrypt=True)
    ...
    def ping_check(self):
        ...
        req = b'1234 Bitraam Plugin 4321'      # free up to 59 bytes
        try:
...
class Coldcard_KeyStore(Hardware_KeyStore):
    ...
    def sign_message(self, sequence, message, password, *, script_type=None):
        ...
        try:
            ...
            # already encoded in Bitraam fashion, binary.
    ...
    def _encode_txin_type(txin_type):
        # Map from Bitraam code names to our code numbers.
...
class ColdcardPlugin(HW_PluginBase):
    ...
    def export_ms_wallet(wallet: Multisig_Wallet, fp, name):
        ...
        print('# Exported from Bitraam', file=fp)
