...
from bitraam.gui.qt.main_window import BitraamWindow
...
class Plugin(ColdcardPlugin, QtPluginBase):
    ...
    @only_hook_if_libraries_available
    @hook
    def wallet_info_buttons(self, main_window: 'BitraamWindow', dialog):
        # user is about to see the "Wallet Information" dialog
        # - add a button if multisig wallet, and a Coldcard is a cosigner.
        assert isinstance(main_window, BitraamWindow), f"{type(main_window)}"
    ...
    def import_multisig_wallet_to_cc(self, main_window: 'BitraamWindow', coldcard_keystores: Sequence[Hardware_KeyStore]):
...
class CKCCSettingsDialog(WindowModalDialog):
    ...
    def __init__(self, window: BitraamWindow, plugin, keystore):
        ...
        assert isinstance(window, BitraamWindow), f"{type(window)}"
        self.window = window
