...
class TrezorKeyStore(Hardware_KeyStore):
    ...
    def add_slip_19_ownership_proofs_to_tx(self, tx: 'PartialTransaction', password) -> None:
        ...
        for txin in tx.inputs():
            ...
            trezor_script_type = self.plugin.get_trezor_input_script_type(desc.to_legacy_bitraam_script_type())
...
class TrezorPlugin(HW_PluginBase):
    ...
    def enumerate(self):
        ...
        # On Windows, the bridge runs as Admin (and Bitraam usually does not),
    ...
    def get_trezor_input_script_type(self, bitraam_txin_type: str):
        if bitraam_txin_type in ('p2wpkh', 'p2wsh'):
            return InputScriptType.SPENDWITNESS
        if bitraam_txin_type in ('p2wpkh-p2sh', 'p2wsh-p2sh'):
            return InputScriptType.SPENDP2SHWITNESS
        if bitraam_txin_type in ('p2pkh',):
            return InputScriptType.SPENDADDRESS
        if bitraam_txin_type in ('p2sh',):
            return InputScriptType.SPENDMULTISIG
        if bitraam_txin_type in ('p2tr',):
            return InputScriptType.SPENDTAPROOT
        raise ValueError('unexpected txin type: {}'.format(bitraam_txin_type))

    def get_trezor_output_script_type(self, bitraam_txin_type: str):
        if bitraam_txin_type in ('p2wpkh', 'p2wsh'):
    ...
    def sign_transaction(self, keystore, tx: PartialTransaction, prev_tx):
        prev_tx = {bfh(txhash): self.bitraam_tx_to_txtype(tx) for txhash, tx in prev_tx.items()}
            ...
            if txin.is_coinbase_input():
                ...
                if for_sig:
                    ...
                    if txin.is_complete() or not txin.is_mine:  # we don't sign
                        ...
                        txinputtype.script_type = self.get_trezor_input_script_type(desc.to_legacy_bitraam_script_type())
    ...
    def tx_outputs(self, tx: PartialTransaction, *, keystore: 'TrezorKeyStore', firmware_version: Sequence[int]):
        ...
        def create_output_by_derivation():
            ...
            assert desc
            script_type = self.get_trezor_output_script_type(desc.to_legacy_bitraam_script_type())
            if multi := desc.get_simple_multisig():
                multisig = self._make_multisig(multi)
            else:
                multisig = None
    ...
    def bitraam_tx_to_txtype(self, tx: Optional[Transaction]):
        t = TransactionType()
        if tx is None:
