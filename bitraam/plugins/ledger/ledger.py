...
class Ledger_Client_New(Ledger_Client):
    ...
    def sign_transaction(self, keystore: Hardware_KeyStore, tx: PartialTransaction, password: str):
        ...
        try:
            ...
            for input_num, (bitraam_txin, psbt_in) in enumerate(zip(tx.inputs(), psbt.inputs)):
                if bitraam_txin.is_coinbase_input():
                ...
                if utxo is None:
                    continue
                if (desc := bitraam_txin.script_descriptor) is None:
                ...
                if is_wit:
                    ...
                    if bitraam_txin.is_p2sh_segwit():
                        if wit_ver == 0:
                ...
                if multisig is not None:
                    ...
                    # Bitraam uses sortedmulti; we make sure that the array of key information is normalized in a consistent order
